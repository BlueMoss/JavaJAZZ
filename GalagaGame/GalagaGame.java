...
class GalagaGame extends JPanel implements KeyListener {
        ...
        public void run() {
            while (true) {
                ...
                if (img_no == 2) // 총을 쏘는 에일리언이면
                    new th2(a, g, bulletImage, img_no, x, y).start();
            } // 추가로 현재 에얼리언의 위치에서 총알을 생성할 스레드 시작
        }
    ...
    private void initSprites() {
        ...
        new th1(this, alienImage, 1, new Random().nextInt(10), new Random().nextInt(10) + 10).start();
        // 총을 안쏘는 에일리언들을 랜덤한 위치에 생성할 스레드 시작
        new th1(this, alienImage2, 2, new Random().nextInt(50), new Random().nextInt(30)).start();
    ...
    public void paint(Graphics g) { // 이미지를 화면에 그려줌
        ...
        if (energy > 0) {
            ...
            g.drawString("남은 독침: " + (int) energy, 600, 500);
        } else {
            g.setColor(Color.red);
            g.drawString("남은 독침: 없음", 600, 500);
        }

        g.setColor(Color.WHITE);
        g.drawString("라이언 학살 점수: " + (int) score, 100, 500);
    }
    ...
    public static void main(String argv[]) {
        GalagaGame g = new GalagaGame(sb);
        g.gameLoop();
    }
