...
public class menu extends JFrame {
   ...
   public menu() {
      ImageIcon icon = new ImageIcon("main3.png");
      JLabel a1 = new JLabel(icon);
      add(a1);
   ...
   class textDialog extends JDialog {
      ...
      public String getInput() {
         if (tf.getText().length() == 0)
            return null; // 텍스트필드에 text가 없으면 널값으로 반환
         else
            return tf.getText(); // 있으면 텍스트를 반환
      }
   }
      ...
      public void actionPerformed(ActionEvent e) {
         ...
         switch (cmd) {// 메뉴 아이템 구분
            case "입력 및 출력":
               ...
               if (text == null)
                  return; // 변수에 아무 입력이 없을경우 그냥 반환,
               label.setText(text);// 있으면 label에 입력된 텍스트 출력

               break;

            case "색상 변경":
               Color selectedColor = JColorChooser.showDialog(null, "Color",
